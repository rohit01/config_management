...
class ConfigManagement(object):
    ...
    TODO
    """
    def __init__(self, filename, logger, mandatory_sections=['msection', 'listx', 'listy'], list_sections=['list1', 'list2'], section_schema=None):
        self.logger = logger
        self.logger.info("Logger initialized in __init__()")
        self.hosts_file = filename
        if mandatory_sections == None:
            self.mandatory_sections = []
        else:
            self.mandatory_sections = mandatory_sections
        if list_sections == None:
            self.list_sections = []
        else:
        ...
        for section_name in self.list_sections:
            if section_name in sections:
                try:
                    ...
                    self.logger.error("'%s' option not specified in mandatory list section '%s'" % (TOTAL_KEY, section_name))
                ...
                for index in range(count):
                    ...
                    try:
                        ...
                        self.logger.error("'%s' section not specified for parent section '%s'. '%s' is a list section with count '%s'."
                            % (child_section, section_name, section_name, count))
                        raise NoSectionError("'%s' section not specified for parent section '%s'. '%s' is a list section with count '%s'."
                            % (child_section, section_name, section_name, count))
    ...
    def update_file(self, updated_host_store):
        ...
        for section_name in self.mandatory_sections:
            if section_name not in all_sections:
                self.logger.error("mandatory section '%s' not present." % (section_name))
                raise KeyError
        del all_sections
        ...
        try:
            with open(self.hosts_file, 'wb') as configfile:
                self.logger.debug("File sucessfully opened: '%s'" % (self.hosts_file))
                config.write(configfile)
                self.logger.info("Config file sucessfully updated: '%s'" % (self.hosts_file))
        except:
            self.logger.exception("Exception in updating config to file: '%s'" % (self.hosts_file))
